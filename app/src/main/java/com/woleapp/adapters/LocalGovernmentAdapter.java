...
public class LocalGovernmentAdapter extends RecyclerView.Adapter<LocalGovernmentAdapter.MyViewHolder>{
    Context context;
    private OnItemClickListener.OnItemClickCallback onClick;

    List<Service> serviceList;
    boolean isPayService = false;
    public LocalGovernmentAdapter(Context context, List<Service> remittanceList, OnItemClickListener.OnItemClickCallback onClick)
    {
    ...
    @Override
    public LocalGovernmentAdapter.MyViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_list_dashboard, parent, false);
        return new LocalGovernmentAdapter.MyViewHolder(view);

    }
    @Override
    public void onBindViewHolder(@NonNull LocalGovernmentAdapter.MyViewHolder holder, int position) {
