...
{
    @Query("select * from business_info WHERE user_id = :id") //WHERE is_deleted = 0
    LiveData<Business> getBusinessInfo(Long id);


    @Query("select * from business_info WHERE business_name LIKE :key") //WHERE is_deleted = 0
    List<Business> searchByBusinessName(String key);
...
//    User getUser(String email);
    ...
    @Query("select COUNT(*) from business_info where user_id = :id")
    LiveData<Integer> doesBusinessInfoExists(int id);
...
//    List<User> findUserByNameAndLastName(String firstName, String lastName);
    ...
    Long insertBusinessInfo(Business business_info);

    @Update(onConflict = REPLACE)
    int updateBusinessInfo(Business business_info);

    @Delete
    int deleteBusinessInfo(Business business_info);
...
//    int deleteMarkedInvoice(int id);
    ...
    @Query("DELETE FROM business_info WHERE user_id = :id ")//DELETE FROM invoice WHERE id = :id AND is_deleted = 1
    int deleteUser(int id);

    @Query("select MAX(user_id) from business_info")
    LiveData<Integer> getMaxID();

    @Query("select COUNT(*) from business_info")//COUNT(id)
    Integer getRowCount();
